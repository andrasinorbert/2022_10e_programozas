...
def masolasTetel(lista, func_felt, func_transform):
    Y=[]
    for i in range(len(lista)):
        if func_felt(lista[i]):
            Y.append(func_transform(lista[i]))
        else:
            Y.append(lista[i])
    return Y
...
def ConvertKisbetuToNagybetu(s):
    return s.upper()

ret=masolasTetel("Szia Feri", kisbetu_e, ConvertKisbetuToNagybetu)
print(ret)